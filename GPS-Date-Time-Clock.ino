/*
* GPS-Based DATE and TIME Clock, v2.0.20240513
* 
...
// This function is used to ensure the local date does not change when the UTC
//   date rolls over to the next day. 
...
void loop() {
  if (gpsSerial.available() > 0) {
    ...
    //   timeZoneOffsetHours accordingly to account for either 4 or 5 hours of 
    //   offset from UTC.
...
void displayDateAndTime(String date, int hours, int minutes, int seconds) {
  ...
  if (hours >= 20 && hours < 24) {
    // If the local time is 8 PM or later, adjust the date
  ...
  // Check each character in the DATE and TIME strings and get their segment
  //   patterns and positions then send them to the MAX7219's registers.                 
  for (int i = 0; i < 8; i++) {
