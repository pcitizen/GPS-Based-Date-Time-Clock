/*
* GPS-Based DATE and TIME Clock, v2.3.2025010
*
* Change v2.4.20250110. Changed 'void setup()' to use Gramin GPS 18X LVC instead of the 
...
// Function to calculate number of days in the year so far this year - Supports Julian Days.
int getNumberOfJulianDays(String date) {
  int year = "2000";
  int month = "01";
  int day = "01";
  // Re-order date string for julian calculation.
  // substring(starting -1, ending)
  if (calendarConfiguration == 1) {
...
// Function to get the days in each month. Adjusts for leapyear too. Thanks chatGPT.
...
void displayDateAndTime(String date, int hours, int minutes, int seconds, int numberOfDays) {
  ...
  if (julianDateConfiguration == 1){
      // julianDateConfiguration switch OFF... display only the 2 digit year and Julian date.
    ...
    if (calendarConfiguration == 1){    
      ...
      if (numberOfDays < 10) {
          julianDays = "000" + date.substring(2, 4) + "00" + String(numberOfDays);
      } else if (numberOfDays < 100){
        julianDays = "000" + date.substring(2, 4) + "0" + String(numberOfDays);
      } else {
        julianDays = "000" + date.substring(2, 4) + String(numberOfDays);
      }
    }
    else {
      // julianDateConfiguration switch ON... display only the Julian date.
      //julianDays = "000" + date.substring(6, 8) + String(numberOfDays);// / 100) + String(numberOfDays % 100);
      if (numberOfDays < 10) {
          julianDays = "000" + date.substring(6, 8) + "00" + String(numberOfDays);
      } else if (numberOfDays < 100){
        julianDays = "000" + date.substring(6, 8) + "0" + String(numberOfDays);
      } else {
        julianDays = "000" + date.substring(6, 8) + String(numberOfDays);
      }
    }
  }
  else {
    ...
    if (numberOfDays < 10) {
        julianDays = "00000" + String(numberOfDays);
    } else if (numberOfDays < 100){
