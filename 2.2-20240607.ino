/*
* GPS-Based DATE and TIME Clock, v2.2-20240607
* 
...
*   the current year and displays this information (Julian Days) as well using 
*   three digits.  Another switch is used to add/removed the current year, 
*   which is appended before the Julian Days. A third switch was added to allow
*   the user to configure the DATE display between YYYYMMDD and MMDDYYYY.
* -----------------------------------------------------------------------------
...
String dateStr = "20240531";
String julianDays = "00123";
...
void adjustDate(String &dateStr, int localHours, int utcHours) {
  ...
  dateStr = String(year) + String(month / 10) + String(month % 10) + String(day / 10) + String(day % 10);
}
...
void loop() {
  ...
  //displayDateAndTime("20240428", 8, 3, 1); // 28 Apr 2024, 8:03:01 AM
}
...
void parseGPSData(String data) {
  if (data.startsWith("$GPRMC")) { // Looking for the $GPRMC string from GPS
    ...
    if (parts[2] == "A") {  // Check to see of the $GPRMC string is valid 
      ...
      // Send all DATE and TIME to displays.
      displayDateAndTime(dateStr, localHours, minutes, seconds, numberOfDays);
    }
  }
}

// Function to calculate number of days in the year so far - Supports Julian Days.
int getNumberOfJulianDays(String date) {
  int year = "2024";
  int month = "01";
  int day = "01";

  // Re-order date string for julian calculation.
  if (calendarConfiguration == 1) {
    year = date.substring(0, 4).toInt(); // Get year part of date
    month = date.substring(4, 6).toInt(); // Get month part of date
    day = date.substring(6, 8).toInt(); // Get day part of date
    }
  else {
    month = date.substring(0, 2).toInt(); // Get month part of date
    day = date.substring(2, 4).toInt(); // Get day part of date
    year = date.substring(4, 8).toInt(); // Get year part of date
  }
    ...
    return daysCount + day; // Add previous month's days plus this month's days.
}

// Array for the days in each month and adjusts for leapyear too. Thanks chatGPT.
...
void initializeMax7219() {
  sendRegister(REG_SCAN_LIMIT, 0x04, REG_SCAN_LIMIT, 0x05, REG_SCAN_LIMIT, 0x07); // Set No. digits
...
//
// When data is sent, is sent one character per display at a time; i.e., the
//   rightmost display's rightmost character is sent, then one for the next
//   display, and then one for the next.  This repeats until eight characters
//   for each display (24 total) are sent.  
// 
// Since the shiftOut command can only handle 8 bits at a time, the command
//   is repeated 2 times the number of displays. Every digit requires one address 
//   byte, and one data byte (16 bits).  So, if there are 16 digits , then 256 
//   bits will be shifted every second, in 8 sets of 32 bits to complete the 
//   displayed data.
// 
// When data is sent to the MAX7219, the register address is sent first, followed 
//   by the address's data.  See page 6 of the MAX7219 datasheet. The last 
//   sentence of "Serial_Addressing Modes" shows that data bit D15 (MSB) is to 
//   be sent first. 
//
...
// This function takes the current data (in segments) and sends it to 
...
void displayDateAndTime(String date, int hours, int minutes, int seconds, 
  ...
  if (hours >= 20 && hours < 24) {
    ...
    //  display; i.e, YYYYMMDD or MMDDYYYY based on the Cal. Config. switch.
    if (calendarConfiguration == 1){
      date = String(year) + String(month / 10) + String(month % 10) + 
            String(day / 10) + String(day % 10);
    }
    else {
      date = String(day / 10) + String(day % 10) + String(month / 10) + 
            String(month % 10) + String(year);
    }
  }

  // Format number of Julian days with preceding two digits for the year plus
  //  three 0s. The zeros are not displayed since the SCAN LIMIT is set to 4. If
  //  Jul. Date Config. switch is on then don't display the year.
  if (julianDateConfiguration == 1){
      // Use Cal Config. switch to know where to get the year from in the DATE string.
    if (calendarConfiguration == 1){    
